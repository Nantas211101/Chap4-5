...
DLL::DLL(StateStack& stack, Context context)
    : State(stack, context),
      mBackgroundSprite(),
      mGUIContainer(),
      mSceneGraph(),
...
bool DLL::handleEvent(const sf::Event& event) {
...
void DLL::resetButton(int size, bool isResetInputButton) {
...
bool DLL::checkInputError(inputID::ID kind, int p) {
    if (kind == inputID::Pos) {
...
void DLL::popingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "")
        usingData1 = data[0];
    int id = toNum(usingData1);
...
void DLL::searchingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "")
        usingData1 = data[0];
...
void DLL::accessingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "")
        usingData1 = data[0];
    int id = toNum(usingData1);
    if (nodeSaver.takeNumOfNode())
...
void DLL::updatingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "") {
        usingData1 = data[0];
        usingData2 = data[1];
    }
