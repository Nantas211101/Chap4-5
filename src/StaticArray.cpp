...
void StaticArray::setStateButton(Context context, int posx, int posy,
...
void StaticArray::setLabel(Context context, int posx, int posy,
...
void StaticArray::setInputButton(Context context, int posx, int posy,
                           const std::string& text,
                           std::function<void()> action) {
    auto stateButton =
...
void StaticArray::resetButton(int size, bool isResetInputButton) {
...
bool StaticArray::checkInputError(inputID::ID kind, int p) {
    if (kind == inputID::Pos) {
...
void StaticArray::pushFrontNode(Context context) {
    if (!nodeSaver.pushMiddleNode(mSceneGraph, data[0], 1, context))
        printedError(context, Constants::outOfSizeError);
}

void StaticArray::pushMiddleNode(Context context, int id) {
    if (!nodeSaver.pushMiddleNode(mSceneGraph, data[1], id, context))
        printedError(context, Constants::outOfSizeError);
}

void StaticArray::pushBackNode(Context context) {
    if (!nodeSaver.pushBackNode(mSceneGraph, data[0], context))
        printedError(context, Constants::outOfSizeError);
}

void StaticArray::popFrontNode(Context context) {
    if (!nodeSaver.popFrontNode(mSceneGraph))
        printedError(context, Constants::outOfSizeError);
}

void StaticArray::popMiddleNode(Context context) {
    if (!nodeSaver.popMiddleNode(mSceneGraph, toNum(data[0])))
        printedError(context, Constants::outOfSizeError);
}

void StaticArray::popBackNode(Context context) {
    if (!nodeSaver.popBackNode(mSceneGraph))
        printedError(context, Constants::outOfSizeError);
}

void StaticArray::pushingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "") {
        if (usingData2 == "") {
            usingData1 = data[0];
            usingData2 = data[1];
...
void StaticArray::popingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "")
        usingData1 = data[0];
    int id = toNum(usingData1);
...
void StaticArray::searchingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "")
        usingData1 = data[0];
...
void StaticArray::accessingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "")
        usingData1 = data[0];
    int id = toNum(usingData1);
...
void StaticArray::updatingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "") {
        usingData1 = data[0];
        usingData2 = data[1];
    }
