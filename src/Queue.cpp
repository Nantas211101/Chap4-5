...
Queue::Queue(StateStack &stack, Context context):
    ...
    mDisplayer(*context.window, 5, textSize, add_x * 2, add_y, context.fonts->get(Fonts::Main)){
        ...
        insertButton->setCallback([this, context, cnty] (){
            ...
            int cntx = 0;
            setStateButton(context, start_x + (++cntx) * add_x, start_y + cnty * add_y, "to First", ValueAction);
            setStateButton(context, start_x + (++cntx) * add_x, start_y + cnty * add_y, "to Middle", ErrorAction);
            setStateButton(context, start_x + (++cntx) * add_x, start_y + cnty * add_y, "to Last", ErrorAction);
...
void Queue::setStateButton(Context context, int posx, int posy, const std::string &text, std::function<void()> action){
...
void Queue::setLabel(Context context, int posx, int posy, const std::string &text, int sizeoftext){
...
void Queue::setInputButton(Context context, int posx, int posy, const std::string &text, std::function<void()> action){
...
void Queue::resetButton(int size){
