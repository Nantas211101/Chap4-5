...
Queue::Queue(StateStack& stack, Context context)
    ...
    insertButton->setCallback([this, context, cnty]() {
        ...
        int cntx = 0;
        setStateButton(context, start_x + (++cntx) * add_x,
                       start_y + cnty * add_y, "Add", pushBackAction);
        setStateButton(context, start_x + (++cntx) * add_x,
    ...
    updateButton->setCallback([this, context, cnty]() {
        ...
        //          start_y + cnty * add_y - add_y / 2, "At position",
        //          textSize);
...
void Queue::setStateButton(Context context, int posx, int posy,
...
void Queue::setLabel(Context context, int posx, int posy,
...
void Queue::setInputButton(Context context, int posx, int posy,
                           const std::string& text,
                           std::function<void()> action) {
    auto stateButton =
...
void Queue::resetButton(int size, bool isResetInputButton) {
...
void Queue::printedError(Context context, const std::string& text,
                         int padding) {
    sf::Vector2f pos = context.window->getView().getCenter();
    setLabel(context, pos.x, pos.y + (padding)*add_y, text, textSize * 2);
}

bool Queue::checkInputError(inputID::ID kind, int p) {
    if (kind == inputID::Pos) {
...
void Queue::accessingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "")
        usingData1 = data[0];
    int id = toNum(usingData1);
...
void Queue::updatingNode(sf::Time dt, const sf::Event& event) {
    if (usingData1 == "") {
        usingData1 = data[0];
        usingData2 = data[1];
    }
