...
World::World(sf::RenderWindow& window)
    : mWindow(window),
      ...
      mWorldBounds(0.f,                            // left X position
                   ...
                   2000.f),                        // height
      mSpawnPosition(mWorldView.getSize().x / 2.f, // X position
                     mWorldBounds.height -
                         mWorldView.getSize().y / 2.f), // Y position
...
void World::buildScene() {
    for (std::size_t i = 0; i < LayerCount; ++i) {
        ...
        mSceneGraph.attachChild(
            std::move(layer)); // attach the layer into the graph
    }
...
void World::setBackground() {
    ...
    backgroundSprite->setPosition(
        ...
        mWorldBounds.top); // set the position (to fullfill the view)
                           // because the backgroundsprite did not set the
                           // origin so its origin is the top left
    mSceneLayers[Background]->attachChild(
        std::move(backgroundSprite)); // attach the child for the background
}
...
void World::setEscort(bool isleft) {
    int tmp = (isleft) ? -1 : 1;
    std::unique_ptr<Aircraft> Escort(new Aircraft(Aircraft::Raptor, mTextures));
...
void World::update(sf::Time dt) {
    ...
    mPlayerAircraft->accelerate(
        0.f, mScrollSpeed); // accelerate according to the ScrollSpeed

    // Regular update step
    mSceneGraph.update(dt);
}

auto World::getCommandQueue() -> CommandQueue& { return mCommandQueue; }
...
void World::setPosition() {
    ...
    // remember the coordinate according to reverse cartesian (similar to the
    // table)
